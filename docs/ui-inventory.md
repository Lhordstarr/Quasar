...
Generated by `node scripts/generate-ui-inventory.mjs` at 2026-08-11T00:34:52.808Z.
...
- Total UI TSX files: 226
- By kind: `component` 144, `story` 20, `modal` 19, `ui` 2, `page` 27, `route-component` 14
- By area: `app` 80, `chat` 16, `common` 25, `input` 12, `knowledge-base` 6, `layout` 7, `message-parts` 2, `model-selector` 5, `session` 3, `settings` 28, `modal` 19, `dev` 7, `guide` 6, `image-creator` 10
- By platform signal: `all` 194, `desktop` 14, `mobile` 20, `web` 3
...
- Text: `Cancel`, `Debug Actions`, `Getting Started`, `Guide debug actions`, `Leave`, `Leave Guide?`, `Quasar Guide`, `Reset Guide`, `Select Language`, `Skip to Completion`, `Stay`, `Switch language`, `Switch Language?`, `Switching language will restart the guide and clear your current progress.`, `Trigger Round Limit`, `Type your Chatbox AI-related question here`
...
- States: `disabled`, `error`, `hasNextPage`, `isCurrentlyGenerating`, `isFetchingNextPage`, `isLoading`, `isRetrying`, `isSmallScreen`, `isTempUpload`, `prompt`, `referenceImages`, `selected`, `selectedModel`, `selectedProvider`, `selectedRatio`, `showHistory`, `showMobileHistory`, `showModelDrawer`, `showRatioDrawer`
...
- States: `error`, `forceShowNewUserScenarioCards`, `hasCompletedFirstSuccessfulChat`, `hasExpiredLicense`, `hasUserSelectedModelRef`, `isLoggedIn`, `isSmallScreen`, `session`
- Variants: None detected
...
- States: `hasExpiredLicense`, `isFetching`, `isSmallScreen`
- Variants: `outline`
...
- Text: `API key invalid!`, `Bing Search is provided for free use, but it may have limitations and is subject to change by Microsoft.`, `BoCha API Key`, `Check`, `Connection successful!`, `DuckDuckGo is provided for free use, but it may have limitations and is subject to change.`, `Enter your Querit API Key`, `Get API Key`, `Max Results`, `Maximum number of results to return.`, `Provided tools`, `Querit API Key`, `Querit Search Options`, `Read Webpage`, `Search Provider`, `Tavily API Key`, `Time Range`, `Time range of the search. For example, the last month.`, `Web Search`
...
- Text: `All data is stored locally, ensuring privacy and rapid access`, `An easy-to-use AI client app`, `Ideal for both work and educational scenarios`, `Quasar`, `Select and configure an AI model provider`, `Setup later`, `Setup Provider`, `Supports a variety of advanced AI models`
...
- States: `actionMenuOpened`, `collapsed`, `disabled`, `error`, `hasFiles`, `hasFoldableProcess`, `isBubbleLayout`, `isCancellableGeneratingAssistantMessage`, `isClassicLayout`, `isClassicMessage`, `isCollapsed`, `isErrorReminder`, `isMessageReminderPresentation`, `isRetryableToolCallStep`, `isRightAlignedMessage`, `isSmallScreen`, `isStreamingMode`, `isThisYear`, `isToday`, `isUserBubble`, `opened`, `processCollapsed`, `retryChoiceOpened`, `success`
...
- Text: `{{count}} lines`, `Advanced Mode`, `Chatbox AI Service`, `Due to local processing limitations, <Link>Chatbox AI Service</Link> is recommended for enhanced document processing capabilities and better results.`, `Due to local processing limitations, <Link>Chatbox AI Service</Link> is recommended to enhance webpage parsing capabilities, especially for dynamic pages.`, `Loading webpage...`, `Local Mode`, `Reading file...`, `Retrying {{attempt}}/{{maxAttempts}}`

### components/chat/MessageMinimapRail

- Source: `src/renderer/components/chat/MessageMinimapRail.tsx`
- Metadata: kind: `component`; area: `chat`
- Components: `BASE_LINE_WIDTH`, `HOVER_DISTANCE`, `HOVER_FALLOFF_POWER`, `ITEM_HEIGHT`, `MAX_LINE_WIDTH`, `MessageMinimapRail`, `PREVIEW_MARGIN`
- Platform signals: `all`
- Platform notes: None detected
- Preview modes: None detected
- Preview links: Missing real fixture
- States: `hasScrollableOverflow`, `hasScrolledToInitialEndRef`, `hoveredAnchor`, `pointerContentY`, `scrollHeight`, `scrollTop`, `viewportHeight`, `visible`
- Variants: None detected
...
- Preview modes: None detected
- Preview links: Missing real fixture
- States: `active`, `collapsed`, `isFavorited`, `isMobile`, `selected`, `visible`
- Variants: `chatbox`, `size:sm`
- Text: `Unavailable`
...
- Preview modes: `desktop-wide`, `narrow/mobile-layout`
- Preview links: Missing real fixture
- States: `activeTab`, `desktopDetail`, `hasRightSideSpace`, `isCustom`, `isFavorited`, `isFavoritedModel`, `isMobile`, `isValidElement`, `mobileDetail`, `mobileOpen`, `search`, `success`, `visible`
...
- Text: `Adjust order`, `Archive`, `Archived. Manage archived chats in Settings.`, `Clean up`, `Manage`, `Pin`, `Too many archived chats`, `Unpin`, `You have archived more than {{count}} chats. Do you want to clean them up now?`
...
- Components: `AUTO_NEW_CHAT_DELAY_SECONDS`, `AutoNewChatLoading`, `FreeTrialLink`, `GUIDE_ACTION_BUTTON_MAX_WIDTH`, `NewChatButton`, `NewChatTip`, `ProviderSettingsButton`
- Platform signals: `all`
- Platform notes: None detected
- Preview modes: None detected
- Preview links: [Storybook: Guide action settings license free trial and new chat states](http://localhost:6006/?path=/story/real-components-guide--guide-action-settings-license-free-trial-and-new-chat-states)
- States: `disabled`, `isFocused`, `isWindowFocused`, `loading`, `pendingExternalAction`, `remainingSeconds`, `visible`
- Variants: `light`, `size:compact-sm`, `size:sm`
- Text: `Claim Free Plan`, `For general conversations, please click`, `New Chat`, `on the side bar to start a new conversation.`, `Opening a new chat...`, `Set up API`, `Sit back, relax. Chatbox will start a new chat in {{count}}s...`, `This is the onboarding assistant`
...
- States: `disabled`, `hasTextContent`, `isAssistant`, `isLastMessage`, `isStreaming`, `isUser`
- Variants: None detected
- Text: None detected
...
- Metadata: kind: `story`; area: `app`; stories: `AvatarStates`, `ChatboxAIErrorMessageStates`, `CompressionModalStates`, `ErrorBoundaryStates`, `InlineUtilityStates`, `ToastStates`
- Components: `AvatarSample`, `SeedLicensePlan`, `SeedToasts`, `SurfaceLabel`, `ThrowOnRender`
- Platform signals: `all`
- Platform notes: None detected
- Preview modes: None detected
- Preview links: [Storybook: Avatar user assistant system chat picture states](http://localhost:6006/?path=/story/real-components-common-feedback--avatar-user-assistant-system-chat-picture-states), [Storybook: Chatbox AI error message quota license file and web browsing states](http://localhost:6006/?path=/story/real-components-common-feedback--chatbox-ai-error-message-quota-license-file-and-web-browsing-states), [Storybook: Compression modal opened confirmation state](http://localhost:6006/?path=/story/real-components-common-feedback--compression-modal-opened-confirmation-state), [Storybook: Error boundary fallback and retry states](http://localhost:6006/?path=/story/real-components-common-feedback--error-boundary-fallback-and-retry-states), [Storybook: Divider link mark scalable icon and segmented control states](http://localhost:6006/?path=/story/real-components-common-feedback--divider-link-mark-scalable-icon-and-segmented-control-states), [Storybook: Toast stack top-right feedback states](http://localhost:6006/?path=/story/real-components-common-feedback--toast-stack-top-right-feedback-states)
- States: `error`, `opened`, `segment`, `success`
- Variants: `size:lg`, `size:sm`, `size:xs`
- Text: `Actual conversation compression confirmation modal used before forced context compaction.`, `Actual global toast renderer backed by the shared UI store.`, `Actual localized Chatbox AI service error copy with settings, plan, homepage, and parser action links.`, `Actual Sentry-backed error boundary with the default app fallback, retry, reload, and details states.`, `Actual user, assistant, system, chat, and picture avatar variants used in session and message rows.`, `Actual utility components used throughout settings, message content, and toolbar controls.`, `Assistant`, `Avatar`, `Avatar user assistant system chat picture states`, `Chat icon`, `Chatbox AI error message quota license file and web browsing states`, `Chatbox homepage link`, `ChatboxAIErrorMessage`, `Compression modal opened confirmation state`, `CompressionModal`, `Divider link mark scalable icon and segmented control states`, `Error boundary fallback and retry states`, `ErrorBoundary`, `Inline common utilities`, `Long support conversation`, `Open compression modal`, `Picture`, `System`, `Toast stack top-right feedback states`, `Toast store is populated; notifications render in the production Snackbar stack.`, `Toasts`, `User`, `Vertical divider`
...
- Platform notes: None detected
- Preview modes: None detected
- Preview links: [Storybook: Guide claim waiting card polling and escape actions](http://localhost:6006/?path=/story/real-components-guide--guide-claim-waiting-card-polling-and-escape-actions), [Storybook: Guide action settings license free trial and new chat states](http://localhost:6006/?path=/story/real-components-guide--guide-action-settings-license-free-trial-and-new-chat-states), [Storybook: Guide message user assistant streaming and tool part states](http://localhost:6006/?path=/story/real-components-guide--guide-message-user-assistant-streaming-and-tool-part-states), [Storybook: Guide suggested questions active and disabled states](http://localhost:6006/?path=/story/real-components-guide--guide-suggested-questions-active-and-disabled-states), [Storybook: Guide user type cards active disabled and selected states](http://localhost:6006/?path=/story/real-components-guide--guide-user-type-cards-active-disabled-and-selected-states)
- States: `active`, `claimStarted`, `disabled`, `isLastMessage`, `isStreaming`, `lastQuestion`, `selected`, `selectedType`
- Variants: `size:sm`, `size:xs`
- Text: `ActionButton`, `Active`, `Actual guide action components for provider settings, new chat, onboarding tip, and free trial claim.`, `Actual guide chat message renderer with user bubble, assistant markdown, streaming loader, and rendered tool parts.`, `Actual onboarding user-type card group with recommended state, hover-ready actions, disabled state, and selected state.`, `Actual suggested guide questions with active chip buttons and disabled previous-message state.`, `Actual waiting card shown after the free-plan claim page opens, including progress indicator, reopen, and skip actions.`, `ClaimWaitingCard`, `Disabled`, `Free trial`, `Guide action settings license free trial and new chat states`, `Guide claim waiting card polling and escape actions`, `Guide message user assistant streaming and tool part states`, `Guide suggested questions active and disabled states`, `Guide user type cards active disabled and selected states`, `GuideMessage`, `New chat`, `New chat tip`, `Provider settings`, `Selected`, `Start a new chat`, `SuggestedQuestions`, `UserTypeCards`
...
- Components: `AgentModeRewardClaimSuccessFixture`, `LoadedSettingsModal`, `ModalPreview`, `OpenModal`, `SeedSampleSession`, `SettingsModal`, `SettingsModalRef`, `SettingsModalRouter`, `StorybookAnalyticsStub`, `SurfaceLabel`
- Platform signals: `all`
- Platform notes: None detected
- Preview modes: None detected
