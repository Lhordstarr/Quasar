...
export function RouteComponent() {
  ...
  const checkBocha = async () => {
    if (extension.webSearch.bochaApiKey) {
      ...
      try {
        await new BochaSearch(extension.webSearch.bochaApiKey).search('Quasar')
  ...
  return (
    <Stack p="md" gap="xxl">
        ...
        onChange={(e) =>
          ...
          setSettings({
            extension: {
              ...
              webSearch: {
                ...extension.webSearch,
                provider: e as 'bing' | 'tavily' | 'bocha' | 'querit' | 'duckduckgo',
              },
            },
          })
        }
      ...
      {extension.webSearch.provider === 'duckduckgo' && (
        <Text size="xs" c="chatbox-gray">
          {t('DuckDuckGo is provided for free use, but it may have limitations and is subject to change.')}
        </Text>
      )}
      {/* Tavily API Key */}
